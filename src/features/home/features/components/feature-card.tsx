...
const FeatureCard = ({ i, feature, Icon, className }: FeatureCardProps) => {
   return (
      <motion.div
         ...
         className={`group relative p-7 lg:p-8 2xl:p-9 rounded-3xl bg-surface-dark/80 border border-violet-500/25 overflow-hidden transition-all duration-500 hover:translate-y-[-6px] hover:border-violet-500/50 hover:bg-surface-hover/90 hover:shadow-glow-violet ${className ?? ""}`}
      >
         {/* Background Glow */}
